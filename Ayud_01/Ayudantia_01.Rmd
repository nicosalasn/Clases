...
```{r Repeticiones} 
rep(1,4)
rep(c(1,5,8,2),4) #Si ponemos un caracter, nos va a tomar todos como caracteres
```
...
### Interacción
```{r Interacción} 
mean(rep(min(1,5,2),5)) #varias funciones dentro de una 
...
Titanic[,.(Survived,Age)]  ## También se puede con nombres de variables, se le pone un punto en vez de un c, porque asi las sigue manteniendo como columnas
```
...
table(Titanic[,Embarked]) #me da cuantas veces se repite cada letra del anterior, el vacio es el 2
unique(Titanic[,Sex])
table(Titanic[,Sex])
...
length(unique(Titanic[,Name])) #numero de personas en base de datos, el unique es para que sea por nombre y apellido y no se repitan
```
