...
```{r cargando bases}
casos <- data.table(read_excel("COVID_19_up_to_29_02_2020_cases.xlsx"),stringsAsFactors = F) #F es para que no me asigne valor numerico a otra categoria
...
```{r as.Date}
casos$dateRep <- as.Date(casos$dateRep,"GMT") #GMT ES DE FECHA Y NO NECESARIAMENTE DE HORA
...
```{r merge}
casosymuertes <- merge(casos,muertes, by = c("day","month","year","countriesAndTerritories"), all = T,sort=F) #All me da todos los casos aunque una columna tenga 5 variables y las otras 10 (all.x y all.y son las más específicas)
```
...
```{r rbind}
al_29_03_2020 <- rbind(casosymuertes,mas_dias) #mas dias se une al final, creando una base de datos más grande.
```
...
al_29_03_2020[!complete.cases(al_29_03_2020),]  ## Otro ejemplo, el ! es la negación, por ende me va a dar las que no estan completas

# Cuantas filas con NAs hay
al_29_03_2020[rowSums(is.na(al_29_03_2020))>0,.N] #N indicará el número de filas  
...
```{r % NAs}
apply(al_29_03_2020, 2, function(col){ #el 2 es el margen, es el vector columna para que sea para day,month, etc... sino seria para cada ID, function(col) {}Todo lo que va en la llave va a ser la función a cumplir
  paste0(round(sum(is.na(col))/length(col),5)*100,"%") #Round redondea para que se vea bonito, el "5" es el redondeo, paste0 es para ponerle el %
  })
```
...
```{r completa}
al_29_03_2020_completo <- al_29_03_2020[complete.cases(al_29_03_2020),] #Datos sin NA, por el complete.cases
```
...
chileyvecinos_completa[order(dateRep)]#ordenar segun la columna daterep
chileyvecinos_completa <- chileyvecinos_completa[order(year, month, day)] # Otra alternativa, cambiamos la base de datos
```
