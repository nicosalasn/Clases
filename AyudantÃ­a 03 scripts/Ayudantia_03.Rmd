...
ggplot(data = cat_ocup, aes(x = cae_general_red, y = V1, fill = cae_general_red)) + #el fill da colores, con group no da colores
  geom_bar(stat="identity") #geom_bar que sea por barra
```
...
ggplot(data = Tabla_19_19[mes_central == params$month,], aes(x = cae_general_red, y = V1, fill = cae_general_red)) + 
  ...
  facet_wrap(~sexo,ncol = 2,scales = 'free_y',as.table = T) #facet_wrap para disntinguir, en este caso por sexo, el free y es para que se midan en base a distintos y pero que lleguen a la misma altura, hacerlo por x quedan en distintas alturas
```
...
Tabla_19_19$sexo <- factor(Tabla_19_19$sexo, levels = c(1,2), labels = c("Hombres","Mujeres")) #cambiar 1 y 2 por hombre y mujer
```
...
ggplot(data = Tabla_19_19[mes_central == params$month,], aes(x = cae_general_red, y = V1, fill = cae_general_red)) + 
  geom_bar(stat="identity") +
  scale_y_continuous(labels = number_format(scale = 0.000001, suffix = "M", big.mark = ".", decimal.mark = ",")) + #ajustar valores
  labs(x = "Categoría Ocupacional", y = "N° de Personas", fill = NULL) + #nombre de los ejes, fill es las categorías de agrupar los datos, en este caso Null para que no aparezca el título
  facet_wrap(~sexo,ncol = 2,scales = 'free_y',as.table = T) #Separar por sexo, EL NUMERO DE COLUMNAS ES PARA QUE LAS COMPARE BIEN
```
...
var_sexo[,sexo_anterior := shift(V1, n = 1, type = 'lag'), by = .(sexo)]# n=1 es traerlo 1 hacia el futuro, 1 a la derecha
```
...
```{r variacion}
var_sexo[,sexo_absoluto := round(V1 - sexo_anterior)]#crea una variable dentro de la base de datos redondeada por lo dicho ahí
var_sexo[,sexo_porcentual := round((V1 - sexo_anterior)/sexo_anterior,4)*100]#crea una variable dentro de la base de datos redondeada por lo dicho ahí
```
...
var_sexo$mes_nombre <- factor(var_sexo$mes_central, levels = c(2:12,1), labels =  c("FEB","MAR","ABR","MAY","JUN","JUL","AGO","SEP","OCT","NOV","DIC","ENE")) #nueva variable en base de datos
```
...
var_sexo[, fecha := ifelse(mes_central > 9, var_sexo$mes_central, paste0("0",var_sexo$mes_central))]#agrgar el 0 para las fechas ejemplo: 01-02-2020
...
ggplot(data = var_sexo, aes(x = fecha, y = sexo_absoluto, group = sexo, color = sexo)) + #agrega parametros
  geom_hline(yintercept = 0) + #linea negra de al medio
  geom_line(position = position_identity()) + #agregamos la lineas
  geom_point(position = position_identity()) +#agrega puntitos esteticos para cada valor
  geom_text(aes(x = unique(var_sexo$fecha)[9], y = 0, label = "aumento\n disminución"), colour="gray60") + #identificar aumentos y disminuciones en base a la linea 0
  geom_text(aes(label = sexo_absoluto), color = "black", nudge_y = 2500, size = 3) + #el valor en negrito, 2500 es la lejanía del punto
  scale_y_continuous(labels = number_format(scale = 0.001, suffix = "K", big.mark = ".", decimal.mark = ",")) + 
  scale_x_date(date_labels = "%Y-%b", breaks = unique(var_sexo$fecha)[c(seq(1,12,2))]) + #transformar mes a año y mes
  labs(x = "Categoría Ocupacional", y = "Variación de Personas", color = NULL)
```
...
var_sexo_2 <- ts(var_sexo_2[, .(Hombres,Mujeres)],start = c(2019,3), end =c(2020,1),frequency = 12)#ts = timeseries (similar a data.table)
```
